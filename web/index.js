...
async function run () {
  try {
    ...
    const howBigIsYourPlanet = async key => {
      ...
      console.log(
        `${person.name} lives on a planet w/ a radius of ${planet.size}mi`
      )
    }
    ...
    const samePlanet = async (key1, key2) => {
      ...
      if (person1.planet.equals(person2.planet)) {
        ...
        console.log(
          `${person1.name} is not on the same planet as ${person2.name}`
        )
      }
    }
  ...
  */
    ...
    let db3 = await dagdb.create('inmem')
    let db4 = await dagdb.create('inmem')

    db3 = await db3.set('hello', 'world4').update()
    db4 = await db4.merge(db3).update()

    console.log(await db4.get('hello'))
    // prints "world4"

    // Create at remote
    /* Not implemented
    console.log('Jim5a')
    const db5 = await dagdb.create('/api/0/test-1')
    console.log('Jim5b')
    console.log('Jim create at remote', await db5.get('hello'))
    console.log('Jim5c')
    */
